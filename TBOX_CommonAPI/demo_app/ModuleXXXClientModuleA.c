...
void module_xxx_method_val_test_call_async_callback( uint8_t *err_code, int32_t *out_val)
{
    ...
    printf("clientA:module_xxx_method_val_test_call async %d\n",*out_val);
}

void module_xxx_evtxxx_subscribe_callback(int32_t *out_val)
{
    printf("clientA:Received broadcast:  %d\n",*out_val);
}
...
int main() 
{
    ...
    while(1)
    {
        ...
        uint8_t in_array[] = "module_xxx_method_array_test_call ModuleA", out_array[100];
        ...
        printf("clientA:module_xxx_method_val_test_call sync %d->%d\n", in_val, out_val);
